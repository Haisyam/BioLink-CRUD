...
<body>
  ...
  <div class="container text-center">
    <h1 class="text-white font-jkt font-semibold text-3xl pb-5">Update Header</h1>
  </div>
  <div class="container mx-auto">
    ...
    ?>
        <form action="update.php?id=<?php echo $id; ?>" method="POST" enctype="multipart/form-data">
          ...
          <div class="pb-5">
            <label for="subtitle">Kata Kata</label>
...
<?php
    ...
    if (isset($_POST['submit'])) {
      ...
      if ($image) {
        // Get old image path
        $sql = "SELECT image_path FROM header WHERE id=$id";
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
        $old_image_path = $row['image_path'];

        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
          $sql = "UPDATE header SET title='$title', subtitle='$subtitle', image_path='$image' WHERE id=$id";
        ...
        $sql = "UPDATE header SET title='$title', subtitle='$subtitle' WHERE id=$id";

        if ($conn->query($sql) === TRUE) {
          echo "<script>alert('Header Updated successfully.'); window.location.href='index.php';</script>";
        } else {
          echo "Error: " . $sql . "<br>" . $conn->error;
        }
      }
